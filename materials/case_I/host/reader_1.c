...
int main() {
    ...
    while(1){
        ...
        uint64_t jt = 12;//rand() % (10);
        for (int i = 0; i < printed; i++) {
            if(i == jt){
                test_addrs[i] = target_addrs[5];
                test_index[i] = jt;
                shared_array[i+1] = jt;
                orig_tests[i] = jt;
                continue;
        ...
        for(int j = 0; j<test_num;j++){
            printf("fc,%llx,122923\n", v2p(test_addrs[j]));
        }
        snprintf(mqtt_data, sizeof(mqtt_data), "wudehua_target_addr=11fb18010=101000056_%d@%d",test_num,jt);
        while (1) {
            ...
            for(int j = 0; j<test_num;j++){
                printf("%llx,", v2p(test_addrs[j]));
            }
            
            
            printf("\n");
            for(int j = 0; j<42;j++){
                clflush(target_addrs[j]);
            }
           // wbinvd();
           ...
           // for(int time =0;time< 1000 ;time++){
                for(int j = 0; j<test_num;j++){
                    //printf("-------111111111-------:%llx\n",times[j]);
                    //if (j % 2 == 0) continue;
                    maccess(test_addrs[j]);
                }
            //}
            //printf("-------222\n\n");
            counter++;
            /*if(counter >= 5){
                for(int j = 0; j<test_num;j++){
                    ...
                    uint64_t tmp = test_addrs[j]; test_addrs[j] = test_addrs[jt]; test_addrs[jt] = tmp;
                }
                counter =0;
            }*/
            ...
            if(counter % 500 == 0){
                test_num = test_num + 1;
                uint64_t jt = 12;
                for(int sstest = 0; sstest <= test_num -1;sstest++){
                    if(sstest == jt){
                        test_addrs[sstest] = target_addrs[5];
                        continue;
                    }
                    test_addrs[sstest] = target_addrs[sstest];
                }
                snprintf(mqtt_data, sizeof(mqtt_data), "wudehua_target_addr=11fb18010=101000056_%d@%d",test_num,jt);
            }
            if(test_num >= 41){
