...
class MQTTReceiver:
    def __init__(self, broker_host="localhost", broker_port=1883, username=None, password=None):
        ...
        # 生成唯一的客户端 ID，避免重启时冲突
        client_id = f"mqtt_receiver_{uuid.uuid4().hex[:8]}_{int(time.time())}"
        self.client = mqtt.Client(client_id=client_id, clean_session=True)
        self.n = 0
        self.time = 0
        self.message_count = 0  # 消息计数器
        self.max_messages = 1000  # 最大消息数
        ...
        print(f"Created MQTT client with ID: {client_id}")
    ...
    def on_message(self, client, userdata, msg):
        ...
        try:
            ...
            print(f"Message count: {self.message_count}/{self.max_messages}")
            
            # 检查是否达到 1000 次
            if self.message_count >= self.max_messages:
                print(f"Reached {self.max_messages} messages, restarting...")
                logger.info(f"Reached {self.max_messages} messages, restarting program...")
                # 先断开连接
                self.client.disconnect()
                self.client.loop_stop()
                print("MQTT connection closed, waiting 2 seconds before restart...")
            ...
            if "wudehua_target_addr" in payload:
                ...
                tests_count = int(tests_case.split('_')[1].split('@')[0],10)
                self.mem_addr = int(tests_case.split('_')[1].split('@')[1],10)
                print(f"memory addr is {self.mem_addr:x}")
                self.n = tests_count
                print("tests_count=="+str(tests_count))
                #logger.info(f"{tests_case},{self.mem_addr}")
                tm = TestMem()
                tm.test_normal(tests_count,self.mem_addr)
                #if self.time % 5 == 0:
                print("sfsffsd") 
