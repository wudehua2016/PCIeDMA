...
def prune_one_by_one_con(index:int):
...
def execute_def():
    ...
    os.system(cmd)
    ts_str = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    logger.info(f"{ts_str},start to execute_def ReEngineering0000.py on 97 server.....")
            ...
            if tests_count <= 100:
                ...
                match mode:
                    case 0:#init
                        ...
                        #split_csv_half()
                        ts_str = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
                        logger.info(f"{ts_str},100 initing,data.csv has been split into two parts")
                        mode = 1
                    ...
                    case 1:# 执行结束后，通过采样数据
                        if total >= threadhold:
                            ...
                            if conflict_times <= 1:
                                ...
                                os.system(cmd)
                                ts_str = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
                                logger.info(f"{ts_str},100 start to new loop,data.csv has been split into two parts")
                                split_csv_half()
                                cmd = "cp ./data_part1.csv ./data.csv"
                                os.system(cmd)
                                execute()
                                ...
                                ts_str = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
                                logger.info(f"{ts_str},100 conflict_times > 1,data.csv does not meet requirement, need to change test data")
                        ...
                        if total <= threadhold:
                            ts_str = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
                            logger.info(f"{ts_str},100 total <= threadhold,data.csv does not meet requirement, need to change test data")
                            is_done = True
                            mode = 2
                    case 2:
                        if total >= threadhold:
                            if conflict_times > 1:
                                ...
                                os.system(cmd)
                                ts_str = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
                                logger.info(f"{ts_str},100 data.csv has been prune_obfuscate into two parts")
                                mode = 1
                                is_done = True
                                execute()
                            elif conflict_times <= 1:
                        ...
                        if total <= threadhold:
                            if conflict_times <= 1:
                                ts_str = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
                                logger.info(f"{ts_str},100 total <= threadhold prune index:{pop_index},data.csv does not meet requirement, need to change test data one by one")
                                is_done = True
                                cmd = "cp ../data.csv ./data.csv"
                                os.system(cmd)
                                cmd = "cp ./data.csv ./data_ori.csv"
                                ...
                                ts_str = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
                                logger.info(f"{ts_str},100 total <= threadhold and conflict_times > 1,data.csv need to reprune the data one by one:{pop_index}")
                                is_done = True
                                mode = 3
                                cmd = "cp ../data.csv ./data.csv"
                                os.system(cmd)
                    ...
                    case 3:
                        if pop_index >= threadhold:
                            ts_str = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
                            logger.info(f"{ts_str},100 find the minimal prune index:{pop_index},break to prune one by one")
                            break
                        if conflict_times > 1:
                            ts_str = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
                            logger.info(f"{ts_str},100 total lines fewer than the threadhold and conflict_times > 1, data.csv need to reprune the data:{pop_index}, and test data one by one")
                            pop_index += 1
                            prune_one_by_one(pop_index)
                            is_done = True
                            execute()
                            ...
                            ts_str = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
                            logger.info(f"{ts_str},100 total lines fewer than the threadhold and conflict_times <= 1,data.csv meets requirement,test another lines:{pop_index}")
                            cmd ="cp ../data.csv ./data_ori.csv"
                            os.system(cmd)
                            prune_one_by_one(pop_index)
                            is_done = True
                    ...
                    case 4:
                        pop_index = 0
                        ts_str = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
                        logger.info(f"{ts_str},100 to prune the data by 10 strides from the minimal prune index:{pop_index}, break to prune one by one")
                    ...
                    case 2:
                        ...
                        if total <= threadhold:
                            if conflict_times <= 1:
                                ...
                                os.system(cmd)
                                mode = 3
                                prune_one_by_one_con(pop_index)
                                execute()
                            elif conflict_times > 1:
                                ts_str = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
