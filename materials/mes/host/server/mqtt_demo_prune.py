                ...
                match mode:
                    ...
                    case 1:# 执行结束后，通过采样数据
                        ...
                        if conflict_times <= 1:
                            ...
                            os.system(cmd)
                            ts_str = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
                            logger.info(f"{ts_str},start to new loop,{idx}:data.csv need to prune another element")
                            prune_one_by_one(idx)
                            ...
                            ts_str = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
                            logger.info(f"{ts_str},conflict_times > 1,{idx}:data.csv does not meet requirement, need to change test data")
