...
def prune_one_by_one(index:int):
    ...
    with open(src_file, newline='') as f:
        ...
        logger.info(f"{ts_str},the dataset after pop {index} is: {rows}")
...
def test_data_csv_row_count():
    ...
    if not os.path.exists(file_path):
        ...
        try:
            ...
            print(f"包含标题行: {total_rows} 行")
            print(f"数据行: {data_rows} 行")
                        ...
                        if total >= threadhold:
                            if conflict_times > 1:
                                ...
                                if(is_test):
                                    ...
                                    logger.info(f"{ts_str},is_test=true,data.csv has been prune_obfuscate into two parts")
                                    #is_test = False
                                    mode = 2
                                elif not is_test:
                                    prune_obfuscate()
                                    ...
                                    os.system(cmd)
                                    ts_str = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
                                    logger.info(f"{ts_str},is_test=false,data.csv has been split into two parts")
                                    mode = 1
                                is_done = True
                                execute()
                            elif conflict_times <= 1:
                                ...
                                if(is_test):
                                    ...
                                    is_done = True
                                    ts_str = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
                                    logger.info(f"{ts_str},data.csv has been split_csv_half into two parts") #wdh
                                    mode = 1
                                    is_test = False
                                elif not is_test:
                                    prune_obfuscate()
                                    ts_str = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
                                    logger.info(f"{ts_str},is_test = False data.csv has been prune_one_by_one,index:{pop_index}")
                                    mode = 1
                                execute()
                        if total <= threadhold:
                            if conflict_times <= 1:
