...
if __name__ == "__main__":
    ...
    try:
        ...
        # 打印最终标记为命中的2MB地址空间
        print("\n" + "=" * 70)
        print("CACHE HIT 2MB BLOCKS (using majority_vote method)")
        print("=" * 70)
        
        # 使用majority_vote方法获取最终结果
        block_stats = aggregate_blocks(df, method='majority_vote')
        
        # 筛选出aggregated_value < 100的块（在cache中）
        cache_hit_blocks = block_stats[block_stats['aggregated_value'] < 100].copy()
        cache_hit_blocks = cache_hit_blocks.sort_index()
        
        print(f"\nTotal cache hit blocks: {len(cache_hit_blocks)}")
        print(f"Percentage: {len(cache_hit_blocks) / len(block_stats) * 100:.2f}%\n")
        
        # 打印每个命中的块
        print("Block Index | Physical Address Range          | Votes (Hit/Miss) | Samples")
        print("-" * 80)
        for block_idx in cache_hit_blocks.index:
            phys_addr_start = block_idx * 0x200000
            phys_addr_end = phys_addr_start + 0x2000000 - 1
            
            row = cache_hit_blocks.loc[block_idx]
            in_cache_votes = int(row.get('in_cache_votes', 0))
            not_in_cache_votes = int(row.get('not_in_cache_votes', 0))
            sample_count = int(row['sample_count'])
            
            print(f"{block_idx:11d} | 0x{phys_addr_start:016x} - 0x{phys_addr_end:016x} | "
                  f"{in_cache_votes:3d}/{not_in_cache_votes:3d}      | {sample_count:3d}")
        
        # 保存到文件 0x200000
        output_txt = os.path.join(output_dir, 'cache_hit_blocks.txt')
        with open(output_txt, 'w') as f:
            f.write("CACHE HIT 2MB BLOCKS (using majority_vote method)\n")
            f.write("=" * 80 + "\n\n")
            f.write(f"Total cache hit blocks: {len(cache_hit_blocks)}\n")
            f.write(f"Percentage: {len(cache_hit_blocks) / len(block_stats) * 100:.2f}%\n\n")
            f.write("Block Index | Physical Address Range          | Votes (Hit/Miss) | Samples\n")
            f.write("-" * 80 + "\n")
            
            for block_idx in cache_hit_blocks.index:
                phys_addr_start = block_idx * 0x200000
                phys_addr_end = phys_addr_start + 0x200000 - 1
                ...
                f.write(f"{block_idx:11d} | 0x{phys_addr_start:016x} - 0x{phys_addr_end:016x} | "
                        f"{in_cache_votes:3d}/{not_in_cache_votes:3d}      | {sample_count:3d}\n")
        
        print(f"\nCache hit blocks also saved to: {output_txt}")
        print("=" * 70)
